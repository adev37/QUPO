// frontend/src/components/purchaseOrders/PurchaseOrderPrint.jsx
...
const PurchaseOrderPrint = () => {
  ...
  const grandTotal =
    typeof po.grandTotal === "number" ? po.grandTotal : subTotal + totalGST;
                  ...
                  <table className="w-full table-auto border-collapse text-xs md:text-sm po-table">
                    ...
                    <tbody>
                      {items.map((it, index) => {
                        ...
                        const price = Number(it.price ?? it.unitPrice ?? 0);
                        const gst = Number(it.gst ?? it.gstPercent ?? 0);
                        const gstAmt = ((qty * price * gst) / 100).toFixed(2);
                        const total = (
                          qty * price +
                          (qty * price * gst) / 100
                        ).toFixed(2);
