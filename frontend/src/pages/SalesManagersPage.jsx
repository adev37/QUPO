...
// Form
function SalesManagerForm({ initialValues, onSubmit, onCancel, loading }) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-[10px] text-slate-500">
            If an existing manager has the same email, it will be updated
            automatically.
          </p>
        </div>
      </div>
...
const SalesManagersPage = () => {
  ...
  const [createOrUpdateManager, { isLoading: isCreating }] =
    useCreateOrUpdateSalesManagerMutation();
  ...
  const handleSubmit = async (values) => {
    try {
      if (selectedManager) {
        await updateManager({ id: selectedManager._id, ...values }).unwrap();
      } else {
        await createOrUpdateManager(values).unwrap();
      }
      closeModal();
    } catch (e) {
      console.error("Failed to save sales manager", e);
    }
  };

  const handleDelete = async (manager) => {
    if (!window.confirm(`Delete sales manager "${manager.name}"?`)) return;
    try {
      await deleteManager(manager._id).unwrap();
    } catch (e) {
      console.error("Failed to delete sales manager", e);
    }
  };
    ...
    <div className="px-4 py-4 sm:px-6 sm:py-6">
      ...
      {isError && (
        <div className="mb-3 rounded-md bg-red-50 border border-red-200 px-3 py-2 text-xs text-red-700">
          {error?.data?.message || "Failed to load sales managers."}
          <button
            onClick={() => refetch()}
