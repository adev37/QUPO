...
export const useQuotationForm = (
  ...
  // when editing, once quotation data loads, sync checkbox state with saved terms
  useEffect(() => {
    if (!initialQuotation?.terms) return;
    const existing = initialQuotation.terms || "";
    const selected = termsList.filter((t) => existing.includes(t));
    setCheckedTerms(selected);
  }, [initialQuotation]); // termsList is constant
  ...
  useEffect(() => {
    ...
    // fully reset with mapped defaults so editing shows exactly saved data
    reset(buildDefaults(initialQuotation));
  }, [initialQuotation, reset, defaultCompanyCode]); // safe deps
